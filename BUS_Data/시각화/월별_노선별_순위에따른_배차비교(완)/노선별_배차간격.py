...
for month in range(1, 13):
    ...
    bottom_rank_per_time_saturday = df_saturday.groupby('시간')['순위'].max()
    bottom_3_saturday = pd.DataFrame()
    for time, bottom_rank in bottom_rank_per_time_saturday.items():
        time_data = df_saturday[(df_saturday['시간'] == time) & (df_saturday['순위'] >= bottom_rank - 2)]
        bottom_3_saturday = pd.concat([bottom_3_saturday, time_data])
    ...
    for route in top_3_saturday['노선'].unique():
        ...
        ax1.scatter(x, y, label=f"Top: {route} (Rank: {route_data['순위'].iloc[0]})", s=100, edgecolors='w', linewidth=2, marker='o')
        for i in range(len(x)):
    ...
    for route in bottom_3_saturday['노선'].unique():
        ...
        ax1.scatter(x, y, label=f"Bottom: {route} (Rank: {route_data['순위'].iloc[0]})", s=100, edgecolors='w', linewidth=2, marker='s')
        for i in range(len(x)):
            ax1.text(x.iloc[i], y.iloc[i], f'{y.iloc[i]}', ha='center', va='bottom', fontsize=12, color='black')
    ...
    bottom_rank_per_time_sunday = df_sunday.groupby('시간')['순위'].max()
    bottom_3_sunday = pd.DataFrame()
    for time, bottom_rank in bottom_rank_per_time_sunday.items():
        time_data = df_sunday[(df_sunday['시간'] == time) & (df_sunday['순위'] >= bottom_rank - 2)]
        bottom_3_sunday = pd.concat([bottom_3_sunday, time_data])
    ...
    for route in top_3_sunday['노선'].unique():
        ...
        ax2.scatter(x, y, label=f"Top: {route} (Rank: {route_data['순위'].iloc[0]})", s=100, edgecolors='w', linewidth=2, marker='o')
        for i in range(len(x)):
    ...
    for route in bottom_3_sunday['노선'].unique():
        ...
        ax2.scatter(x, y, label=f"Bottom: {route} (Rank: {route_data['순위'].iloc[0]})", s=100, edgecolors='w', linewidth=2, marker='s')
        for i in range(len(x)):
            ax2.text(x.iloc[i], y.iloc[i], f'{y.iloc[i]}', ha='center', va='bottom', fontsize=12, color='black')
